defmodule Aoc.Day2 do
  ...
  def part2(opts \\ []) do
    ...
    |> Stream.map(&String.to_integer/1)
    |> Stream.chunk_every(2)
    |> Task.async_stream(fn [first, last] ->
      Range.new(first, last)
      |> Stream.map(&Integer.digits(&1))
      |> Stream.flat_map(fn digits ->
        Enum.reduce_while(Enum.with_index(digits, 1), [], fn {_, index}, invalid ->
          case Enum.split(digits, index) do
            {needle, haystack} when length(needle) > length(haystack) ->
              {:halt, invalid}

            {needle, haystack} ->
              haystack
              |> Enum.chunk_every(length(needle))
              |> Enum.map(fn x -> x == needle end)
              |> Enum.all?()
              |> case do
